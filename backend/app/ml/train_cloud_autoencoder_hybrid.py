...
MODEL_PATH = BACKEND_DIR / "models" / "cloud_autoencoder_hybrid.keras"
SCALER_PATH = BACKEND_DIR / "models" / "cloud_autoencoder_hybrid_scaler.joblib"
META_PATH = BACKEND_DIR / "models" / "cloud_autoencoder_hybrid_meta.json"
PUBLIC_DATASET_PATH = BACKEND_DIR / "data" / "public_datasets" / "cloud_public" / "public_cloud_normalized.jsonl"
...
def load_local_cloud_rows() -> list[SecurityEvent]:
    db = SessionLocal()
    try:
        return (
...
def build_local_features(rows: list[SecurityEvent]) -> tuple[list[list[float]], int, int]:
...
def load_public_events() -> tuple[list[dict], int]:
    if not PUBLIC_DATASET_PATH.exists():
        return [], 0

    events: list[dict] = []
    total_rows = 0
    with PUBLIC_DATASET_PATH.open("r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
...
def build_public_features(events: list[dict]) -> list[list[float]]:
    feature_rows: list[list[float]] = []
    ...
    for event in events:
        ...
        try:
            features = extract_cloud_features(normalized_event)
        except Exception:
            continue
...
def main() -> int:
    ...
    try:
        local_rows = load_local_cloud_rows()
    except Exception as exc:
        print(f"[ERROR] Failed to load local cloud rows from database: {exc}")
        return 1

    public_events, public_total_rows = load_public_events()
    local_total_cloud_rows = len(local_rows)
    local_features, excluded_lookupevents, excluded_awsservice_assumerole = build_local_features(local_rows)
    public_features = build_public_features(public_events)
    combined_features = local_features + public_features

    local_usable_rows = len(local_features)
    public_usable_rows = len(public_features)
    combined_usable_rows = len(combined_features)

    print(f"[INFO] Local cloud rows read: {local_total_cloud_rows}")
    print(f"[INFO] Local usable rows: {local_usable_rows}")
    print(f"[INFO] Local excluded LookupEvents rows: {excluded_lookupevents}")
    print(f"[INFO] Local excluded AWSService AssumeRole rows: {excluded_awsservice_assumerole}")
    print(f"[INFO] Public normalized rows read: {public_total_rows}")
    print(f"[INFO] Public usable rows: {public_usable_rows}")
    print(f"[INFO] Combined usable rows: {combined_usable_rows}")

    if combined_usable_rows < MIN_TRAINING_ROWS:
        print(f"[ERROR] Need at least {MIN_TRAINING_ROWS} usable combined cloud rows to train safely.")
        return 1
    ...
    try:
        ...
        metadata = {
            "model_name": "autoencoder_cloud_hybrid_v1",
            "source_profile": "cloud_hybrid",
            "feature_dim": feature_dim,
            "local_total_cloud_rows": local_total_cloud_rows,
            "local_usable_rows": local_usable_rows,
            "local_excluded_lookupevents_rows": excluded_lookupevents,
            "local_excluded_awsservice_assumerole_rows": excluded_awsservice_assumerole,
            "public_total_rows": public_total_rows,
            "public_usable_rows": public_usable_rows,
            "combined_usable_rows": combined_usable_rows,
        ...
        print(f"[ERROR] Hybrid cloud autoencoder training failed: {exc}")
        return 1

    print(f"[INFO] Saved hybrid cloud autoencoder to: {MODEL_PATH}")
    print(f"[INFO] Saved hybrid cloud scaler to: {SCALER_PATH}")
    print(f"[INFO] Saved hybrid cloud metadata to: {META_PATH}")
    return 0
